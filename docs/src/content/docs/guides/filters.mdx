...
public class VisitedFilter implements JsonFilter<MyState> {
    ...
    @Override
    public LuxisPipeline<Void> handle(HttpStream<Void, MyState> e) {
...
A filter's stream is the **same `HttpStream` type** handlers receive, so the entire pipeline API is available: `validate()`, `validateHttp()`, `requireJwt()`, `map()`, `flatMap()`, `blockingMap()`, `blockingFlatMap()`, `asyncMap()`, `peek()`, and so on. You can short-circuit a request (e.g. return `401` from `flatMap`) or enrich the downstream context before the route handler runs.
...
- Query parameters — `ctx.session().getQueryParam("...")` (and via `v.queryParam(...)` in `validateHttp()`)
- Path parameters — `ctx.session().getPathParam("...")` (and via `v.pathParam(...)` in `validateHttp()`)
...
Because `validateHttp()` lives on `HttpStream`, filters can also validate query and path parameters before requests reach the route — but they cannot validate body fields (there is no body object to validate).

```java
return e
    .validateHttp(v -> v.queryParam("tenant").required().matches("[a-z0-9-]+"))
    .complete();
```