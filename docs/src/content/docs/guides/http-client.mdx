...
Point the stub client at another `LuxisTest.from(...)` instance. Requests execute in-process with no network:

```java
// The service you want to call
TestLuxis<UserState> userService = LuxisTest.from(Luxis.app(UserApp::registerRoutes));

// Create a client that talks to it in-memory
LuxisHttpClient httpClient = StubLuxisHttpClient.create(userService);
```

`LuxisTest` and `StubLuxisHttpClient` are part of `luxis-test-support`. See the [testing guide](../testing/) for the dependency block.
...
TestLuxis<AppState> orderService = LuxisTest.from(Luxis.<AppState>app(r -> {
    AppState state = new AppState();
