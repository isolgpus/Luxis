...
Point the stub client at another `Luxis.app(routes).test()` instance. Requests execute in-process with no network:

```java
// The service you want to call
Luxis<UserState> userService = Luxis.app(UserApp::registerRoutes).test();
...
Luxis<AppState> orderService = Luxis.<AppState>app(r -> {
    AppState state = new AppState();
