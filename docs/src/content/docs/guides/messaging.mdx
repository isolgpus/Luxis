...
The same `EventPlatform` registers the publish side (`Publisher` + `OutboxStore`) and the consume side (`EventConsumer`). All three are optional within the platform — register only the sides you use.
