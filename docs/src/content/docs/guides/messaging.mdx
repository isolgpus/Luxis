...
When the platform is registered, Luxis starts a periodic drainer (`pollIntervalMillis`, `batchSize`) and kicks it after every successful commit so events ship as soon as possible.
...
- Owns a periodic drainer (one per Luxis instance, started on `.start()`, stopped on `close`).
...
Luxis can also subscribe to a single topic and route incoming messages to typed pipelines via `eventRoute(...)`. You provide an `EventConsumer` that knows how to talk to your broker and decode the wire format; Luxis dispatches each message to the matching route.

One `EventConsumer` per Luxis instance, one topic per consumer. Routes within that topic are differentiated by a routing key extracted from each message (e.g. a JSON envelope's `"type"` field, a Kafka header, an SBE template id).

### Registering routes

```java
r.eventRoute("orderPlaced", appState, OrderPlaced.class, stream -> stream
    .map(ctx -> ctx.in())                       // ctx.in() is the decoded OrderPlaced
    .blockingPeek(ctx -> ordersDao.record(ctx.in()))
    .complete());
```

`eventRoute` mirrors `jsonRoute` but the terminal stage produces no response — events have no reply. Pipelines support the full DSL (`map`, `flatMap`, `blockingMap`, `asyncMap`, `validate`, `inTransaction`, `ctx.publisher()`, …) with the same threading rules as HTTP routes.

Duplicate keys throw `IllegalArgumentException` at registration. Messages whose key has no registered route are reported via the configured `exceptionHandler` and dropped; the same applies to decode failures.
...
- `topic()` — informational; documents which topic this consumer is bound to.
- `extractKey(buf)` — pure function from the raw message to the routing key. Treat the buffer as read-only; `duplicate()` if you advance positions.
- `decode(buf, type)` — the codec strategy. One wire format per topic, so the consumer owns deserialization. Routes declare just `Class<T>` and `ctx.in()` is typed `T`. If you really want raw bytes, register the route with `ByteBuffer.class` and have `decode` return the buffer.
- `start(dispatcher)` — Luxis calls this after routes are registered. Subscribe to the broker and call `dispatcher.dispatch(buf)` per message.
- `close()` — Luxis calls this on shutdown to unsubscribe and release resources.
...
The same `EventPlatform` registers the publish side (`Publisher` + `OutboxStore`) and the consume side (`EventConsumer`). `EventPlatform.of(publisher, outboxStore, eventConsumer)` requires all three to be non-null — if a service only publishes or only consumes, supply no-op implementations for the unused side rather than passing `null`.
...
| `EventConsumer.extractKey` throws | Reported via `exceptionHandler`; message dropped. |
| Incoming message has no matching `eventRoute` key | `IllegalStateException` reported via `exceptionHandler`; message dropped. |
| `EventConsumer.decode` throws | Reported via `exceptionHandler`; message dropped, pipeline does not run. |
| Event-route pipeline throws or returns a failure | Reported via `exceptionHandler` (event routes have no client to reply to). |
