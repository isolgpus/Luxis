...
WebSocket clients honour the same stub-or-real split as the rest of the HTTP client. `StubLuxisHttpClient` connects to the target `Luxis.app(routes).test()` instance in-process with no network, while `VertxLuxisHttpClient` opens a real WebSocket — the client code is identical either way.
