...
The blocking context is the same `RestrictedBlockingRouteContext` HTTP handlers see — neither `ctx.session()` nor `ctx.app()` is reachable from a worker thread, and the type system enforces this. WebSocket write operations (such as sending messages) must run on the event loop that owns the connection, so the framework keeps you off the worker for those.
...
As your WebSocket route grows, extract handlers into separate methods. WebSocket pipelines are built on `LuxisStream` and terminate in a `LuxisPipeline`, the same types HTTP and event-route pipelines use — there is no dedicated `WebSocketStream` / `WebSocketPipeline`:

```java
public class ChatWebSocketRoutes extends WebSocketRoutes<AppState, ChatResponse> {

    private LuxisPipeline<MessageResponse> handleSendMessage(
            LuxisStream<SendMessageRequest, AppState, ChatResponse, ErrorMessageResponse, WebSocketSession<ChatResponse>> stream) {
        return stream
    ...
    private LuxisPipeline<StatusResponse> handleGetStatus(
            LuxisStream<StatusRequest, AppState, ChatResponse, ErrorMessageResponse, WebSocketSession<ChatResponse>> stream) {
        return stream
                .map(ctx -> new StatusResponse(ctx.app().onlineUsers()))
                .complete();
    }
...
WebSocket testing works the same way as HTTP — in-memory with `TestLuxis.from(Luxis.app(routes))`, or against a real server with `Luxis.app(routes).start()`. The `TestLuxis` factory and stub clients ship in `luxis-test-support` (see the [testing guide](../testing/) for the dependency block):
...
public class ChatWebSocketTest {
    ...
    @Before
    public void setUp() {
        luxis = TestLuxis.from(Luxis.app(MyApp::registerRoutes));
...
```java
luxis = Luxis.app(MyApp::registerRoutes)
...
routesRegister.webSocketRoute("/ws/chat", state, new ChatWebSocketRoutes(),
    new WebSocketRouteConfigBuilder()
        .corruptInputStrategy(new SendErrorResponse("Bad message format"))
        .failedValidationStrategy(JustSendValidationError.INSTANCE)
        .backpressureStrategy(BackpressureStrategy.DISCONNECT_CLIENT)
        .build());
```

- **Corrupt input** — what happens when the client sends invalid JSON or an unknown `type`. Default: `DisconnectSession.INSTANCE` (disconnect).
- **Failed validation** — what happens when validation fails. Default: `JustSendValidationError.INSTANCE` (send the validation errors and keep the connection open).
- **Backpressure** — what happens when the write queue is full. Default: `BackpressureStrategy.UNBOUNDED_BUFFER` (buffer all the messages).
