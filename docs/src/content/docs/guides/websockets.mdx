...
WebSocket testing works the same way as HTTP — in-memory with `LuxisTest.from(Luxis.app(routes))`, or against a real server with `Luxis.app(routes).start()`. The `LuxisTest` factory and stub clients ship in `luxis-test-support` (see the [testing guide](../testing/) for the dependency block):
