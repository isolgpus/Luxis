...
|---|---|---|
| **Factory** | `LuxisTest.from(Luxis.app(routes))` | `Luxis.app(routes).start()` |
...
Luxis<MyState> luxis = LuxisTest.from(Luxis.app(MyApp::registerRoutes));
TestClient client = new StubTestClient("127.0.0.1", 8080, luxis);
```

No server starts. No ports are opened. Your handlers execute synchronously in-process. `LuxisTest.from(...)` accepts the same `LuxisBuilder` you'd configure for production — `.withConfig(...)`, `.withDatabase(...)`, `.withEventPlatform(...)` all work identically before you hand the builder over.
...
public class EchoTest {
    ...
    @Before
    public void setUp() {
        luxis = LuxisTest.from(Luxis.app(MyApp::registerRoutes));
...
TestLuxis<DownstreamState> downstream = LuxisTest.from(Luxis.app(DownstreamApp::registerRoutes));
...
TestLuxis<UpstreamState> upstream = LuxisTest.from(Luxis.<UpstreamState>app(r -> {
...
In stub mode, all of this runs in-memory. To switch to real HTTP, replace `LuxisTest.from(...)` with `.start()` on the builder and `StubLuxisHttpClient` with `VertxLuxisHttpClient`. The test assertions don't change.
