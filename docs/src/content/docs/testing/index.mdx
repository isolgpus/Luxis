...
The test harness lives in a separate artifact, `luxis-test-support`. Add it to your `pom.xml` with `<scope>test</scope>` so it never reaches your production classpath:

```xml
<dependency>
    <groupId>io.kiw.luxis</groupId>
    <artifactId>luxis-test-support</artifactId>
    <version>0.5.2</version>
    <scope>test</scope>
</dependency>
```

`luxis-test-support` already depends on `luxis`, so you don't need to declare it twice in your test classpath.

Everything below comes from the `io.kiw.luxis.web.test` package. Anything you find under `io.kiw.luxis.web.test.internal` is wiring used by the harness itself — it is not part of the supported API and may change between versions.

## Two modes, one test

| | In-memory (stub) | Real server (Vert.x) |
|---|---|---|
| **Factory** | `TestLuxis.from(Luxis.app(routes))` | `Luxis.app(routes).start()` |
...
Define your routes in a method that implements `ApplicationRoutesRegister<APP>`. This is the same function your production server calls:
...
```java
Luxis.app(MyApp::registerRoutes).start();
```
...
Luxis<MyState> luxis = TestLuxis.from(Luxis.app(MyApp::registerRoutes));
TestClient client = new StubTestClient("127.0.0.1", 8080, luxis);
```

No server starts. No ports are opened. Your handlers execute synchronously in-process. `TestLuxis.from(...)` accepts the same `LuxisBuilder` you'd configure for production — `.withConfig(...)`, `.withDatabase(...)`, `.withEventPlatform(...)` all work identically before you hand the builder over.

### Real server (Vert.x)

```java
Luxis<MyState> luxis = Luxis.app(MyApp::registerRoutes)
...
TestLuxis<DownstreamState> downstream = TestLuxis.from(Luxis.app(DownstreamApp::registerRoutes));
...
TestLuxis<UpstreamState> upstream = TestLuxis.from(Luxis.<UpstreamState>app(r -> {
...
In stub mode, all of this runs in-memory. To switch to real HTTP, replace `TestLuxis.from(...)` with `.start()` on the builder and `StubLuxisHttpClient` with `VertxLuxisHttpClient`. The test assertions don't change.
