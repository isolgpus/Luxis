...
|---|---|---|
| **Factory** | `Luxis.app(routes).test()` | `Luxis.app(routes).start()` |
...
```java
Luxis<MyState> luxis = Luxis.app(MyApp::registerRoutes).test();
...
public class EchoTest {
    ...
    @Before
    public void setUp() {
        luxis = Luxis.app(MyApp::registerRoutes).test();
...
Luxis<DownstreamState> downstream = Luxis.app(DownstreamApp::registerRoutes).test();
...
Luxis<UpstreamState> upstream = Luxis.<UpstreamState>app(r -> {
...
In stub mode, all of this runs in-memory. To switch to real HTTP, replace `.test()` with `.start()` on the builder and `StubLuxisHttpClient` with `VertxLuxisHttpClient`. The test assertions don't change.
